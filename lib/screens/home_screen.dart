...
class HomeScreenState extends State<HomeScreen> {
  ...
  String _getMachoMessage() {
    const pool = [
      ...
      'お前の継続、ガチで神ぽん💪 筋肉は裏切らないぽん。',
      '連続達成とかお前しか勝たんぽん💪 継続はバフぽん。',
    ];
  ...
  String _getPontaMessage() {
    ...
    if (_todayCalories == 0) {
      pool = [
        ...
        '記録ゼロとかエアプぽん。記録ガチ勢になるぽん。',
        'ワンチャン何も食べてない説？…ないぽんね。書くぽん。',
      ];
    } else if (ratio <= 0.5) {
      pool = [
      ...
      pool = [
        ...
        '夜の冷蔵庫はラスボスぽん。ノーダメで抜けるぽん。',
        'ここで間食したら台無しフラグぽん。回避するぽん。',
      ];
    } else if (ratio < 1.0) {
      pool = [
        'ギリギリだぽん。あと少し、踏ん張るぽん。',
        ...
        '瀬戸際ぽん。冷蔵庫に近づくの禁止ぽん。',
        'ここからの一口は無理ゲーぽん。今日はもう店じまいぽん。',
        'デザートはワンチャンいけると思った？ないぽん。',
      ];
    } else if (ratio < 1.2) {
      pool = [
        ...
        '「実質ゼロカロリー」は通用しないぽん。数字は正直ぽん。',
        'オーバーは草…いや笑えないぽん。明日取り返すぽん。',
      ];
    } else {
      pool = [
        ...
        '大草原ぽんwww 今日の食事、伝説になったぽん。',
        'ここまでいくと飯テロ側ぽん。はい解散、明日ガチるぽん。',
      ];
    }
